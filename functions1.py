...
def get_response(entity_type=["benzinera"]):
    """[summary]
...
def get_id_by_entityType(entity_type=["benzinera"]):
    ...
    """
    response = get_response(entity_type)
    ids_list = []
    ids = {}
...
def get_data_by_id(id, entity_type="benzinera"):
    """
...
def get_df_row_by_id(gasolinera_id, entity_type="benzinera"):
    ...
    #check here
    dicc = get_data_by_id(gasolinera_id, entity_type)
...
def update_prices(entity_type = "benzinera"):
    ...
    """
    id_gasolineras_list = get_id_by_entityType([entity_type])[1]
    csv_name = 'bonarea_gasolineras_prices1.csv'
...
def update_data_and_prices(entity_type = "benzinera"):
    ...
    """
    id_gasolineras_list = get_id_by_entityType([entity_type])[1]
    csv_name = 'bonarea_gasolineras_data_and_prices1.csv'
    if os.path.isfile(csv_name):
        with open(csv_name,'a', newline='') as csvfile:
            for i in range(0,len(id_gasolineras_list)):
...
def update_dataframe(entity_type = ["benzinera"]):
    ...
    id_gasolineras_list = get_id_by_entityType(entity_type)[0]
    ...
    if os.path.isfile(csv_name):
        ...
        with open(csv_name,'a', newline='') as csvfile:
            colname = pd.DataFrame(get_df_row_by_id('A', 'benzinera')[0].columns.tolist()).transpose()
            csvfile.write(colname.to_csv(index=False, header=False))
            for k, v in id_gasolineras_list.items():
                csvfile.write(get_df_row_by_id(k, v)[0].to_csv(index=False, header=False))
